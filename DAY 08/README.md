...
- Functions with multiple parameters and meaningful return values
- Using the modulo operator (`%`) to wrap around the end of a list/alphabet
- Merging similar functions (encrypt/decrypt) into one using a conditional branch
- `try`/`except` for validating user input without crashing
- Splitting a project across multiple files (`main.py`, `art.py`) with `import`
